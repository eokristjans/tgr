...
\href{https://stackoverflow.com/questions/3341485/how-to-make-a-html-page-in-a4-paper-size-pages}{Einhverjar
góðar upplýsingar um page-styling}

    \section{Verkefni II -\/- Motion Control in Computer-Aided
Modeling}\label{verkefni-ii----motion-control-in-computer-aided-modeling}
...
progress curves may be required:Ahand reaching for a doorknob might
...
Assume that a parametric path P = \(\{x(t),y(t) | 0 \leq t \leq 1\}\) is
given. Figure 5.6 shows the example path
...
{\color{incolor}In [{\color{incolor}3}]:} \PY{k}{def} \PY{n+nf}{computeArcLength}\PY{p}{(}\PY{n}{T}\PY{p}{)}\PY{p}{:}
            \PY{k}{if}\PY{p}{(}\PY{n}{T}\PY{o}{\PYZgt{}}\PY{l+m+mi}{1}\PY{p}{)}\PY{p}{:}
                \PY{n+nb}{print}\PY{p}{(}\PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{Wrong parameter passed. Call function with parameter T\PYZlt{}=1}\PY{l+s+s1}{\PYZsq{}}\PY{p}{)}
                \PY{k}{return} \PY{o}{\PYZhy{}}\PY{l+m+mi}{1}
            \PY{n}{t1}\PY{o}{=}\PY{l+m+mi}{0}
            \PY{n}{t2}\PY{o}{=}\PY{n}{T}
            \PY{c+c1}{\PYZsh{} Höldum áfram hér. Adaptive Quadrature er kafli 5.4 í bókinni.}
\end{Verbatim}
...
the arc length from \(t = 0\) to \(t = 1\) should be equal to \(s\). Use
the Bisection Method to locate the point \(t^∗(s)\) to three correct
decimal places.
